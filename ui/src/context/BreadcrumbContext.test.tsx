...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Verrail", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Verrail",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Verrail");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Verrail");
    expect(buildDocumentTitle([], null)).toBe("Verrail");
  });
});
