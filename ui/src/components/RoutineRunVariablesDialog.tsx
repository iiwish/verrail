      ...
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routines.runDialog.agentRequired")}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routines.group.agent")}
                noneLabel={t("routines.runDialog.selectAgent")}
                searchPlaceholder={t("routines.runDialog.searchAgents")}
                emptyMessage={t("routines.runDialog.noAgents")}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routines.runDialog.selectAgent")}</span>
                  )
                }
                renderOption={(option) => {
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("routines.runDialog.defaultAgentRequired")}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routines.runDialog.missing", { names: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routines.runDialog.chooseWorkspace")}
            </p>
          ) : (
          ...
          >
            {isPending ? t("routines.row.running") : t("routines.runDialog.run")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
