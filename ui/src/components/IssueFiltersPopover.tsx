  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("tasks.filters.count", { count: activeFilterCount }) : t("tasks.filters.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("tasks.filters.count", { count: activeFilterCount }) : t("tasks.filters.filter")}</span>}
            ...
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {t(`tasks.filters.quickValues.${preset.label.toLowerCase()}`)}
                  </button>
                );
              })}
            </div>
              ...
              <div className="space-y-1">
                ...
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("tasks.noResponsible")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("tasks.me")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
                ...
                <div className="space-y-1">
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("tasks.filters.noCreators")}</div>
                    )}
                  </div>
                </div>
              ) : null}
                    ...
                    {externalObjectFilterOrder.map((value) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm">{t(`tasks.filters.externalValues.${value}`)}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-1">
                ...
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("tasks.filters.hideRoutine")}</span>
                  </label>
                ) : null}
              </div>
            </div>
