        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("tasks.progress.done", { done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("tasks.progress.inProgress", { count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("tasks.progress.blocked", { count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={t("tasks.progress.costTitle", {
                    runCount: costSummary.runCount.toLocaleString(),
                    issueCount: costSummary.issueCount,
                  })}
                >
                  {t("tasks.progress.tokens", { count: formatTokens(totalTokens) })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("tasks.progress.runtime", { duration: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("tasks.progress.ariaLabel")}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={`${t(`tasks.status.${status}`)}: ${count}`}
                aria-hidden="true"
              />
            ))}
          </div>
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("tasks.progress.nextUp") : t("tasks.progress.waitingBlockers")}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("tasks.progress.noActive")}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("tasks.progress.allDone")}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("tasks.progress.noActionable")}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("tasks.board") : t("tasks.me"),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return issueStatusOrder
        ...
        .map((s) => ({ key: s, label: t(`tasks.status.${s}`), items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return issuePriorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: t(`tasks.priority.${p}`), items: groups[p]! }));
    }
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("tasks.groups.noWorkspace") : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("tasks.groups.noProject") : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("tasks.groups.noParent") : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("tasks.user"))
  ...
  const createActionLabel = createIssueLabel ? t("tasks.createNamed", { name: createIssueLabel }) : t("tasks.create");
  const createButtonLabel = createIssueLabel ? t("tasks.newNamed", { name: createIssueLabel }) : t("tasks.new");
    ...
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("tasks.sort.title")}>
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("tasks.group.title")}>
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("tasks.searchLimit", { count: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                ({t("tasks.subtaskCount", { count: totalDescendants })})
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <Badge variant="ghost"
                                  ...
                                  aria-label={t("tasks.paused")}
                                  title={t("tasks.paused")}
                                >
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <Badge variant="outline"
                                ...
                                title={t("tasks.needsNextStepDescription")}
                              >
                                  ...
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? t("tasks.user")}
                    ...
                    for (const crossedBucket of prevBucket === null
                      ...
                      nodes.push(
                        <IssueDateSeparator
                          ...
                          label={t(crossedBucket === 1 ? "tasks.olderThanDay" : "tasks.olderThanWeek")}
                        />,
                      );
                    }
                    prevBucket = bucket;
              ...
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ...
                  : remainingIssueRowCount > 0
                    ? t("tasks.renderingCount", { count: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length })
                    : t("tasks.scrollMore")}
              </p>
            </div>
          )}
