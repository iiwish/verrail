...
export function issueActivityText(issue: Issue): string {
  return i18n.t("tasks.updatedAgo", {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          {!iconOnly && t("tasks.columns.title")}
        </Button>
      </DropdownMenuTrigger>
          ...
          >
            <span className="flex flex-col gap-0.5">
              <span className="text-sm font-medium text-foreground">
                {t(`tasks.columns.labels.${column}`)}
              </span>
              <span className="text-xs leading-relaxed text-muted-foreground">
                {t(`tasks.columns.descriptions.${column}`)}
              </span>
            </span>
          </DropdownMenuCheckboxItem>
        ))}
    ...
    <>
      ...
      {showSubtreeLiveChip && !isLive && subtreeLiveCount > 0 && (
        <Badge variant="outline"
          ...
          title={t("tasks.runningBelowTitle", { count: subtreeLiveCount })}
        >
          <span
            className={cn(
          ...
          <span className="hidden text-(length:--text-micro) font-medium text-muted-foreground sm:inline">
            {t("tasks.liveBelow", { count: subtreeLiveCount })}
          </span>
        </Badge>
      )}
    </>
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("tasks.user");
  const originatingActor = deriveOriginatingActor(issue);
  const originatingUserId = originatingActor?.kind === "user" ? originatingActor.id : null;
  const creatorUserLabel = creatorUserName ?? formatAssigneeUserLabel(originatingUserId, currentUserId) ?? t("tasks.user");
      ...
      {columns.map((column) => {
        ...
        if (column === "kickedOffBy") {
          ...
          if (originatingActor?.kind === "user") {
            const tooltipText = viaAgentName ? t("tasks.viaAgent", { user: creatorUserLabel, agent: viaAgentName }) : creatorUserLabel;
            return (
              <Tooltip key={column}>
                <TooltipTrigger asChild>
                  <span className="min-w-0 text-xs text-foreground">
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("tasks.subtask")}</span>
              )}
            </span>
          );
        }
