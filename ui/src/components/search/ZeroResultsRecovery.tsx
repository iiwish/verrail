  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <div className="text-base font-semibold">{t("search.filters.noResults")}</div>
        <p className="text-sm text-muted-foreground">
          {t("search.filters.unfilteredMatches", { count: unfilteredTotal })}
          {query ? <> &ldquo;{query}&rdquo;</> : null}, but your{" "}
          {t("search.filters.hiddenBy", { count: activeCount })}
        </p>
      </div>
          ...
          {suggestions.map((suggestion) => (
            ...
            >
              ...
              <span className="shrink-0 tabular-nums text-emerald-600 dark:text-emerald-400">
                {t("search.filters.additionalResults", { count: suggestion.additionalCount })}
              </span>
            </button>
          ))}
        </div>
