...
function statusMeta(entry: WorkspaceServiceControlEntry, t: TFunction): { label: string; unhealthy: boolean } {
  switch (entry.state) {
    case "provisioning":
      return { label: t("workspace.services.status.provisioning"), unhealthy: false };
    case "starting":
      return { label: t("workspace.services.status.starting"), unhealthy: false };
    case "stopping":
      return { label: t("workspace.services.status.stopping"), unhealthy: false };
    case "restarting":
      return { label: t("workspace.services.status.restarting"), unhealthy: false };
    case "failed":
      return { label: t("workspace.services.status.failed"), unhealthy: false };
    case "running":
      return entry.healthStatus === "unhealthy"
        ? { label: t("workspace.services.status.unhealthy"), unhealthy: true }
        : { label: t("workspace.services.status.running"), unhealthy: false };
    default:
      return { label: t("workspace.services.status.stopped"), unhealthy: false };
  }
}
...
function CopyUrlButton({ url, disabled }: { url: string; disabled?: boolean }) {
  ...
  const copyLabel = copyState === "copied"
    ? t("workspace.services.urlCopied")
    : copyState === "failed"
      ? t("workspace.services.copyFailed")
      : t("workspace.services.copyUrl");
  return (
    <Button
      variant="ghost"
      size="icon-xs"
...
function UrlSegment({ entry, compact }: { entry: WorkspaceServiceControlEntry; compact?: boolean }) {
  ...
  if (!displayUrl) {
    return <span className="font-mono text-xs text-muted-foreground/70">{t("workspace.services.noUrl")}</span>;
  }
  return (
    <>
      {live ? (
      ...
      <span className={cn("flex items-center", live ? null : "invisible")} aria-hidden={live ? undefined : true}>
        ...
        <Button
          ...
          disabled={!live}
          className="text-muted-foreground hover:text-foreground"
          title={t("workspace.services.openNewTab")}
        >
          {live ? (
            <a href={entry.url ?? undefined} target="_blank" rel="noreferrer" aria-label={t("workspace.services.openNewTab")}>
  ...
  if (entry.state === "stopped") {
    return (
      <Button
        ...
        aria-label={t("workspace.services.actions.start")}
        title={t("workspace.services.actions.start")}
      >
        <Play className="size-3" />
        {t("workspace.services.actions.start")}
      </Button>
    );
  }
    ...
    return (
      <>
        ...
        <Button
          ...
          aria-label={t("workspace.services.actions.restart")}
          title={t("workspace.services.actions.restart")}
  ...
  return (
    <>
      <Button
        ...
        aria-label={t("workspace.services.actions.stop")}
        title={t("workspace.services.actions.stop")}
        className="border border-border text-foreground"
      >
...
function ServicePopoverRow({
  ...
  const secondary = entry.exposureDetail
    ...
    : entry.state === "starting" && entry.port
      ? t("workspace.services.startingOnPort", { port: entry.port })
...
function MultiServiceBar({
  ...
  const aggregateEntry: WorkspaceServiceControlEntry = {
    key: "__all__",
    name: t("workspace.services.allServices"),
        ...
        <div className="flex h-9 items-center pl-3 pr-1.5">
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <button
                ...
                aria-label={t("workspace.services.runningSummaryAria", { running: runningCount, total: services.length })}
              >
                <StatusIndicator entry={aggregateEntry} />
                <span className="whitespace-nowrap">{t("workspace.services.runningSummary", { running: runningCount, total: services.length })}</span>
            ...
            <PopoverContent align="end" className="w-96 p-0" onOpenAutoFocus={(event) => event.preventDefault()}>
              <div className="px-4 pb-1 pt-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                {t("workspace.services.count", { count: services.length })}
              </div>
              ...
              <div className="flex items-center gap-1 border-t border-border px-4 py-2">
                <Button variant="ghost" size="xs" onClick={() => onAction("start", null)}>{t("workspace.services.actions.startAll")}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("stop", null)}>{t("workspace.services.actions.stopAll")}</Button>
                <Button variant="ghost" size="xs" onClick={() => onAction("restart", null)}>{t("workspace.services.actions.restartAll")}</Button>
                {onManageServices ? (
                  <Button
                    variant="link"
                    size="xs"
          ...
          <div className="hidden min-w-0 items-center gap-0.5 sm:flex">
            {primary ? (
              ...
              <span className="font-mono text-xs text-muted-foreground/70">{t("workspace.services.noUrl")}</span>
            )}
          </div>
          <div className="mx-3 hidden h-5 w-px bg-border sm:block" />
          <div className="ml-auto flex items-center gap-1 pl-3 sm:pl-0">
