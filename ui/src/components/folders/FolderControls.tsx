...
export function FolderChip({
  ...
  return (
    <Button variant="outline" size="sm" className="max-w-full justify-start" onClick={onClick}>
      ...
      <span className="truncate">{selection === "unfiled" ? t("folders.unfiled") : selectionLabel({ folders: result?.folders ?? [], selection, allLabel })}</span>
...
export function FolderRail({
  ...
  return (
    <nav aria-label={t("folders.navigation", { items: itemLabelPlural })} className="hidden w-(--sz-folder-rail) shrink-0 border-r border-border pr-3 md:block">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("folders.title")}</div>
        <Button variant="ghost" size="icon-sm" title={t("folders.new")} onClick={onCreate}>
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      <span className="min-w-0 flex-1 text-muted-foreground">
        {t("folders.suggestion", { items: itemLabelPlural })}
      </span>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-folder-sheet-max) rounded-t-lg pb-4">
        <SheetHeader className="border-b border-border px-4 py-3">
          <SheetTitle>{t("folders.navigation", { items: itemLabelPlural })}</SheetTitle>
        </SheetHeader>
...
export function MoveToMenu({
  ...
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger>{t("folders.moveTo")}</DropdownMenuSubTrigger>
  ...
  return (
    <>
        ...
        {visibleFolders.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("folders.noMatches")}</div>
        ) : null}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onCreateAndMove}>
          <Plus className="h-3.5 w-3.5" />
          {t("folders.newEllipsis")}
        </DropdownMenuItem>
    </>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? t("folders.edit") : t("folders.create")}</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="folder-name">{t("common.name")}</label>
            <Input
              id="folder-name"
              value={name}
            ...
            <div className="flex flex-wrap gap-2">
              {FOLDER_COLORS.map((swatch) => (
                <button
                  key={swatch}
                  type="button"
                  aria-label={t("folders.useColor", { color: swatch })}
                  className={cn(
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSubmit({ name: name.trim(), color })} disabled={pending || !name.trim()}>
            {pending ? t("common.saving") : isEdit ? t("common.save") : t("folders.create")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t("folders.deleteDescription", { count: folder?.itemCount ?? 0, items: itemLabelPlural })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          >
            {pending ? t("common.deleting") : t("folders.delete")}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export function BulkBar({
  ...
  return (
    <div className="sticky top-2 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <span className="mr-auto text-sm text-muted-foreground">{t("folders.selected", { count: selectedCount })}</span>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="sm" variant="outline">{t("folders.moveTo")}</Button>
        </DropdownMenuTrigger>
