...
export function ApprovalCard({
  ...
  const kindLabel = t(`approvals.types.${approval.type}`, { defaultValue: typeLabel[approval.type] ?? approval.type });
      ...
      <div className="flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent && (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{t("approvals.detail.requestedBy")}</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("approvals.card.created", { time: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 py-1 text-xs text-muted-foreground">
            ...
            <span>{t(`statuses.${approval.status}`, { defaultValue: approval.status.replace(/_/g, " ") })}</span>
          </div>
        </div>
      </div>
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("approvals.card.approving") : t("approvals.detail.approve")}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("approvals.card.rejecting") : t("approvals.detail.reject")}
                </Button>
              </>
            )}
          </div>
