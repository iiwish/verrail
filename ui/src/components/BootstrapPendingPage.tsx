  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? "A bootstrap invite is already active. Check your Verrail startup logs for the first-admin URL, or run this command on the host to rotate it:"
          : "Run this command on the host that runs Verrail to print a one-time first-admin invite URL:"}
      </p>
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <Card className="block p-6">
        <VerrailBrand className="mb-6 h-5" />
        {children}
      </Card>
    </div>
  );
...
export function BootstrapPendingPage({
  ...
  if (!claimAvailable) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">This Verrail instance is waiting on its first admin</h1>
        <p className="mt-2 text-sm text-muted-foreground">
  ...
  if (!session) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">Finish setting up Verrail</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          No admin has claimed this instance yet. Sign in or create your Verrail account to become the first
