...
export function approvalLabel(type: string, payload?: Record<string, unknown> | null): string {
  const base = i18n.t(`approvals.types.${type}`, { defaultValue: typeLabel[type] ?? type });
...
function SkillList({ values }: { values: unknown }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("approvals.payload.skills")}</span>
