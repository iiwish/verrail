...
function formatDayLabel(dateStr: string): string {
  ...
  return new Intl.DateTimeFormat(getCurrentLocale(), { month: "numeric", day: "numeric" }).format(d);
}
...
function runDayTooltip(entry: DashboardRunActivityDay, t: TFunction): string {
  const lines = [t("dashboard.charts.tooltip.runs", { date: entry.date, count: entry.total })];
  if (entry.succeeded > 0) lines.push(t("dashboard.charts.tooltip.succeeded", { count: entry.succeeded }));
  if (entry.recovered > 0) lines.push(t("dashboard.charts.tooltip.recovered", { count: entry.recovered }));
  if (entry.failed > 0) {
    lines.push(t("dashboard.charts.tooltip.failed", { count: entry.failed }));
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(t("dashboard.charts.tooltip.other", { count: entry.other }));
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("dashboard.charts.noRuns")}</p>;

  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("statuses.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("dashboard.charts.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("statuses.failed") },
    { color: runSegmentColors.other, label: t("dashboard.charts.other") },
  ];
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("dashboard.charts.tooltip.tasks", { date: day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`tasks.priorities.${p}`) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: t(`statuses.${s}`, { defaultValue: s }) }))} />
    </div>
  );
}
