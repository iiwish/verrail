...
export function formatLastRunTimestamp(value: Date | string | null | undefined, neverLabel = "Never") {
      ...
      {selectMode ? (
        ...
        >
          <input
            ...
            checked={selected}
            aria-label={t("routines.row.select", { title: routine.title })}
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {t(`statuses.${isArchived ? "archived" : isDraft ? "draft" : "paused"}`)}
            </span>
          ) : null}
          {managedByLabel ? (
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-2">
            ...
            />
            <span>{routine.projectId ? (project?.name ?? t("routines.groupLabels.Unknown project")) : t("routines.noProject")}</span>
          </span>
          <span className="flex items-center gap-2">
            {agent?.icon ? <AgentIcon icon={agent.icon} className="h-3.5 w-3.5 shrink-0" /> : null}
            <span>{routine.assigneeAgentId ? (agent?.name ?? t("routines.groupLabels.Unknown agent")) : t("routines.row.noDefaultAgent")}</span>
          </span>
          <span>
            {formatLastRunTimestamp(routine.lastRun?.triggeredAt, t("routines.row.never"))}
            {routine.lastRun?.status ? ` · ${t(`statuses.${routine.lastRun.status}`, { defaultValue: formatRoutineRunStatus(routine.lastRun.status) ?? routine.lastRun.status })}` : ""}
          </span>
        </div>
        {secondaryDetails ? (
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        ...
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("routines.row.disable", { title: routine.title }) : t("routines.row.enable", { title: routine.title })}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("statuses.archived") : isDraft ? t("statuses.draft") : enabled ? t("routines.row.on") : t("routines.row.off")}
          </span>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link to={href}>{configureLabel ?? t("routines.row.edit")}</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={runDisabled}
              onClick={() => onRunNow(routine)}
            ...
            <DropdownMenuItem
              ...
              disabled={isStatusPending || isArchived || disableToggle}
            >
              {enabled ? t("routines.row.pause") : t("routines.row.enableAction")}
            </DropdownMenuItem>
