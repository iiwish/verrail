...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = t(isPaused ? "sidebarAgents.resumeAgent" : "sidebarAgents.pauseAgent");
  ...
  const trailingLabel = [
    showBuiltInLifecycle
      ? t("sidebarAgents.builtInStatus", {
          status: t(`sidebarAgents.builtInStatuses.${builtInStatus}`),
        })
      : null,
          ...
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle
                ...
                aria-label={t("agents.invalidReportingChain")}
              />
            ) : null}
          </span>
        ) : undefined
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>
                  {t(starred ? "sidebarAgents.removeFromStarred" : "sidebarAgents.starAgent")}
                </span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("sidebarAgents.budgetPauseReason") : undefined}
          >
          ...
          >
            ...
            <span>{t(leaving ? "sidebarAgents.leaving" : "sidebarAgents.leaveAgent")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const agentSortChoices: SidebarSectionRadioChoice[] = [
    { value: "top", label: t("sidebarAgents.sortModes.top") },
    { value: "alphabetical", label: t("sidebarAgents.sortModes.alphabetical") },
    { value: "recent", label: t("sidebarAgents.sortModes.recent") },
  ];
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: t(action === "pause" ? "sidebarAgents.paused" : "sidebarAgents.resumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: t(action === "pause" ? "sidebarAgents.pauseFailed" : "sidebarAgents.resumeFailed"),
