...
import { Camera, Globe2, LoaderCircle, Save, Trash2, UserRoundPen } from "lucide-react";
...
export function ProfileSettings() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profile.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings.profile.uploadRequiresCompany"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profile.uploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profile.removeFailed"));
    },
  });
  ...
  const uploadHint = selectedCompany
    ? t("settings.profile.storedInCompany", { company: selectedCompany.name })
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings.profile.photoHint", { storageHint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="space-y-3" aria-labelledby="profile-language-heading">
          <div className="flex items-center gap-2">
            <Globe2 className="size-4 text-muted-foreground" />
            <h2 id="profile-language-heading" className="text-sm font-medium">
              {t("settings.profile.language.title")}
            </h2>
          </div>
          <p className="text-sm text-muted-foreground">
            {t("settings.profile.language.description")}
          </p>
          <div className="space-y-2">
            <Label htmlFor="profile-language">{t("settings.profile.language.label")}</Label>
            <Select
              value={currentLocale}
              onValueChange={(value) => {
