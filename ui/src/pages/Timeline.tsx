...
function rangeError(range: DateRangeState): "missing" | "order" | null {
  if (!range.fromDate || !range.toDate) return "missing";
  if (!rangeWindow(range)) return "order";
  return null;
}
...
function TimelineSummaryStats({
  ...
  const stats: { label: string; value: string; icon: LucideIcon }[] = [
    { label: t("timeline.runs"), value: formatInteger(summary.runs), icon: GanttChartSquare },
    { label: t("agents.title"), value: formatInteger(summary.agents), icon: Bot },
    { label: t("timeline.runTime"), value: formatDuration(0, summary.activeMs), icon: Clock3 },
    {
      label: t("timeline.tokensUsed"),
...
export function Timeline() {
  ...
  const dateRangeErrorCode = rangeError(dateRange);
  const dateRangeError = dateRangeErrorCode ? t(`timeline.rangeError.${dateRangeErrorCode}`) : null;
  ...
  const header = (
    <div className="flex items-center gap-2">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">{t("timeline.heading")}</h1>
    </div>
  );
    ...
    <label className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
      ...
      <Segmented
        ...
        options={[
          ...
          { value: "7d", label: t("timeline.sevenDays") },
          { value: "30d", label: t("timeline.thirtyDays") },
        ]}
      />
      <Input
        type="date"
        ...
        aria-label={t("timeline.startDate")}
      />
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      ...
      <div className="ml-auto flex items-center gap-1 pt-3" aria-label={t("timeline.zoomControls")}>
        <Button
          type="button"
          variant="outline"
          size="icon-xs"
  ...
  return (
    <div className="space-y-6">
      ...
      {data && !isLoading && !dateRangeError && (
        data.spans.length === 0 ? (
          <div className="space-y-3">
            <EmptyState icon={GanttChartSquare} message={t("timeline.empty")} />
            <div className="flex flex-wrap items-center justify-end gap-3">
              {rangeControls}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                ...
                {new Date(data.window.from).toLocaleString(getCurrentLocale())} {t("timeline.to")} {new Date(data.window.to).toLocaleString(getCurrentLocale())}
