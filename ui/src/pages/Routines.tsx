...
export function Routines() {
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: routine.assigneeAgentId
          ? t("routines.toasts.createdAddTrigger")
          : t("routines.toasts.createdDraft"),
        tone: "success",
      });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            title: t("routines.toasts.folderCreatedMoveFailed"),
            body: moveError instanceof Error ? moveError.message : t("routines.errors.moveSelected"),
            tone: "error",
          });
          return;
        }
      ...
      pushToast({ title: t("routines.toasts.folderCreated"), body: folder.name, tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toasts.folderSaveFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.saveFolder"),
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toasts.folderUpdateFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.updateFolder"),
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ title: t("routines.toasts.folderDeleted"), body: t("routines.toasts.itemsMovedUnfiled"), tone: "success" });
    },
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toasts.folderDeleteFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.deleteFolder"),
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toasts.moveFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.moveRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.updateRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.errors.startRun"),
        tone: "error",
      });
    },
  });
  ...
  async function moveSelectedRoutines(folderId: string | null) {
    ...
    try {
      ...
      pushToast({ title: t("routines.toasts.moved"), body: t("routines.toasts.movedCount", { count: ids.length }), tone: "success" });
    } catch (moveError) {
      pushToast({
        title: t("routines.toasts.moveManyFailed"),
        body: moveError instanceof Error ? moveError.message : t("routines.errors.moveSelected"),
        tone: "error",
      });
    }
            ...
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.sort.title")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.sort.title")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{t(`routines.sort.${labelKey}`)}</span>
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.group.title")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.group.title")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{t(`routines.group.${labelKey}`)}</span>
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.new")}</p>
              <p className="text-sm text-muted-foreground">
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("routines.advanced.description")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.advanced.concurrency")}</p>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {concurrencyPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.advanced.concurrencyPolicies.${value}.label`)}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">{t(`routines.advanced.concurrencyPolicies.${draft.concurrencyPolicy}.description`)}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-(--tracking-caps) text-muted-foreground">{t("routines.advanced.catchUp")}</p>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {catchUpPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.advanced.catchUpPolicies.${value}.label`)}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">{t(`routines.advanced.catchUpPolicies.${draft.catchUpPolicy}.description`)}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.errors.create")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines.errors.load")}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div className={cn(showFolderRail && "flex gap-4")}>
          {showFolderRail ? (
            <FolderRail
              ...
              selection={folderSelection}
              allLabel={t("routines.all")}
              itemLabelPlural={t("routines.itemLabelPlural")}
          ...
          {routineViewState.groupBy === "folder" && hasRoutineFolders ? (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {folderSelection === "all" ? <FolderIconHeader label={t("routines.all")} countLabel={t("routines.count", { count: sortedRoutines.length })} /> : (
                <div className="flex min-w-0 items-center gap-2 text-sm">
                  <FolderSwatch color={activeFolder?.color} />
                  <span className="truncate font-medium">{folderSelection === "unfiled" ? t("routines.unfiled") : activeFolder?.name ?? t("routines.group.folder")}</span>
                  <span className="text-muted-foreground">{t("routines.count", { count: sortedRoutines.length })}</span>
                </div>
              )}
            </div>
          ) : null}
          ...
          {visibleRoutines.length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines.empty.active")}
              />
            </div>
            ...
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={folderSelection === "all" ? t("routines.empty.view") : t("routines.empty.folder")}
              />
              {folderSelection !== "all" ? (
                <div className="mt-3 flex justify-center">
                  <Button size="sm" onClick={openCreateRoutine}>
                    <Plus className="mr-2 h-3.5 w-3.5" />
                    {t("routines.newInFolder")}
                  </Button>
                </div>
              ) : null}
            </div>
                  ...
                  >
                    {group.label ? (
                      <RoutineSectionHeader
                        label={t(`routines.groupLabels.${group.label}`, { defaultValue: group.label })}
                          ...
                          extraMenuItems={
                            <MoveToMenu
                              ...
                              onMove={(folderId) => {
                                ...
                                pushToast({
                                  title: t("routines.toasts.movedOne"),
                                  body: folderId
                                    ? t("routines.toasts.movedToFolder", { title: routine.title, folder: routineFolders?.folders.find((folder) => folder.id === folderId)?.name ?? t("routines.group.folder") })
                                    : t("routines.toasts.movedToUnfiled", { title: routine.title }),
                                  tone: "success",
                                  action: {
                                    label: t("common.undo"),
