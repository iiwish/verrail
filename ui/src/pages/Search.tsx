      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {t(`search.scopes.${value as CompanySearchScope}`)}
            {dashOut ? (
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            ...
            <span className="truncate">
              {t("search.operatorHintPrefix")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">status:todo</code>,{" "}
              <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">assignee:me</code>,{" "}
              {t("search.operatorHintOr")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">updated:&gt;7d</code>.
            </span>
          )}
        </div>
      </div>
...
function SearchTabContent({
  ...
  const { t } = useTranslation();
  const scopeLabel = t(scope === "all" ? "search.allScopes" : `search.scopes.${scope}`);
    ...
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.identifierLookup")}</span>{" "}
            {t("search.identifierLookupPrefix")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code>{" "}
            {t("search.identifierLookupSuffix")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.quotedPhrases")}</span>{" "}
            {t("search.quotedPhrasesDescription")}
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span> {t("search.commandPaletteDescription")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.errorTitle")}</div>
        <p className="text-sm text-muted-foreground">
          {status ? t("search.serverError", { status }) : t("search.requestFailed")} {t("search.retryDescription")}
        </p>
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.searching", { query: trimmedQuery })}
        </div>
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.noResultsDescription", { scope: scopeLabel })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? t("search.resultRange", { count: totalResults, total: allMatchTotal })
            : totalResults === 1
              ? t("search.resultCount", { count: 1 })
              : t("search.resultCount", { count: totalResults })}
          {t("search.sortedBy", { sort: sortLabel })}
          {activeFilterCount > 0
            ? t("search.activeFilters", { count: activeFilterCount })
            : ""}
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{t("common.updating")}</span> : null}
      </div>
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={t(`search.scopes.${group.key}`)}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={t(`search.scopes.${group.key}`)}
                trailing={
