...
describe("Secrets page layout", () => {
  ...
  it("uses the shared search/filter/tab affordances and keeps vault sections quiet", async () => {
    ...
    expect(container.textContent).toContain("Bind secrets to runtime environment variables.");
  ...
  it("warns that removing a provider vault only removes Verrail config", async () => {
    ...
    expect(document.body.textContent).toContain("from Verrail only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Verrail"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
