...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.instance.heartbeats.title") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.heartbeats.errors.update"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("settings.instance.heartbeats.errors.disableOne", { detail })
            : t("settings.instance.heartbeats.errors.disableMany", { failed: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.heartbeats.errors.disableAll"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.instance.heartbeats.errors.load")}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>{t("settings.instance.heartbeats.activeCount", { count: activeCount })}</span>
        <span>{t("settings.instance.heartbeats.disabledCount", { count: disabledCount })}</span>
        <span>{t("settings.instance.heartbeats.companyCount", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("settings.instance.heartbeats.disable") : t("settings.instance.heartbeats.enable")}
                          </Button>
                        </span>
                      </div>
                    );
