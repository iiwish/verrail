...
export function RoutineDetail() {
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) result.push({ key: "title", label: t("routines.detail.dirty.title") });
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: t("routines.detail.dirty.description") });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: t("routines.detail.dirty.project") });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: t("routines.detail.dirty.defaultAgent") });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: t("routines.detail.dirty.priority") });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: t("routines.detail.dirty.concurrency") });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: t("routines.detail.dirty.catchUp") });
    }
    if (editDraft.activityGatePolicy !== routineDefaults.activityGatePolicy) {
      result.push({ key: "activityGatePolicy", label: t("routines.detail.dirty.advancedPolicy") });
    }
    if (editDraft.activityGateScope !== routineDefaults.activityGateScope) {
      result.push({ key: "activityGateScope", label: t("routines.detail.dirty.gateScope") });
    }
    if (JSON.stringify(editDraft.variables) !== JSON.stringify(routineDefaults.variables)) {
      result.push({ key: "variables", label: t("routines.detail.dirty.variables") });
    }
    if (JSON.stringify(editDraft.env ?? null) !== JSON.stringify(routineDefaults.env ?? null)) {
      result.push({ key: "env", label: t("routines.detail.dirty.secrets") });
    }
    return result;
  ...
  const copySecretValue = useCallback(
    async (label: string, value: string) => {
      try {
        ...
        pushToast({
          title: t("routines.detail.toasts.copyFailed", { label }),
          body: copyError instanceof Error ? copyError.message : t("routines.detail.errors.clipboardDenied"),
          tone: "error",
        });
      }
    },
  ...
  const saveRoutine = useMutation({
    ...
    onError: (mutationError) => {
      if (mutationError instanceof ApiError && mutationError.status === 409) {
        ...
        pushToast({
          title: t("routines.detail.toasts.changed"),
          body: t("routines.detail.toasts.changedBody"),
          tone: "warn",
        });
        return;
      }
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.detail.errors.save"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (runError) => {
      pushToast({
        ...
        body: runError instanceof Error ? runError.message : t("routines.detail.errors.run"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routines.detail.toasts.saved"),
        body: status === "paused" ? t("routines.detail.toasts.paused") : t("routines.detail.toasts.enabled"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (statusError) => {
      pushToast({
        ...
        body: statusError instanceof Error ? statusError.message : t("routines.detail.errors.update"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("routines.detail.toasts.webhookCreated"),
          entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
        });
      } else {
        pushToast({ title: t("routines.detail.toasts.triggerAdded"), body: t("routines.detail.toasts.scheduleSaved"), tone: "success" });
      }
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        ...
        body: triggerError instanceof Error ? triggerError.message : t("routines.detail.errors.createTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routines.detail.toasts.triggerSaved"), body: t("routines.detail.toasts.cadenceSaved"), tone: "success" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.activity(selectedCompanyId!, routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        ...
        body: triggerError instanceof Error ? triggerError.message : t("routines.detail.errors.updateTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routines.detail.toasts.triggerDeleted"), tone: "success" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.activity(selectedCompanyId!, routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        ...
        body: triggerError instanceof Error ? triggerError.message : t("routines.detail.errors.deleteTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("routines.detail.toasts.webhookRotated"),
        entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("routines.detail.toasts.webhookRotateFailed"),
        body: triggerError instanceof Error ? triggerError.message : t("routines.detail.errors.rotateWebhook"),
        tone: "error",
      });
    },
  });
      ...
      setSecretMessage({
        title:
          response.secretMaterials.length === 1
            ? t("routines.detail.toasts.webhookRestored")
            : t("routines.detail.toasts.webhooksRestored", { count: response.secretMaterials.length }),
  ...
  if (error || !routine || !routineDefaults) {
    return (
      <EmptyState
        icon={AlertCircle}
        message={error instanceof Error ? error.message : t("routines.detail.errors.load")}
      />
    );
  }
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <ToggleSwitch
                ...
                aria-label={automationEnabled ? t("routines.detail.pauseTriggers") : t("routines.detail.enableTriggers")}
              />
          ...
          >
            ...
            >
              <h2 id="routine-section-title" className="mb-4 text-lg font-semibold">
                {t(`routines.detail.sections.${section}`)}
              </h2>
