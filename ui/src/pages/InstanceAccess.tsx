...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.instance.title"), href: "/company/settings/instance/general" },
      { label: t("settings.instance.access.title") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const setAdminMutation = useMutation({
    mutationFn: async (makeAdmin: boolean) => {
      if (!selectedUserId) throw new Error(t("settings.instance.access.errors.noUser"));
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("settings.instance.access.toasts.roleUpdated"), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.instance.access.loadingUsers")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("settings.instance.access.errors.adminRequired")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("settings.instance.access.errors.loadUsers");
      ...
      <div className="grid gap-6 lg:grid-cols-(--gtc-34)">
        <Card className="block space-y-4 p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("settings.instance.access.searchUsers")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("settings.instance.access.searchPlaceholder")}
            />
          </label>
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("settings.instance.access.activeMemberships", { count: user.activeCompanyMembershipCount })}
                </div>
              </button>
            ))}
          </div>
        ...
        <Card className="block space-y-4 p-5">
          {!selectedUserId ? (
            ...
            <div className="text-sm text-muted-foreground">{t("settings.instance.access.loadingAccess")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("settings.instance.access.errors.loadAccess")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? t("settings.instance.access.removeAdmin") : t("settings.instance.access.promoteAdmin")}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("settings.instance.access.companyAccess")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("settings.instance.access.companyAccessDescription")}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("common.saving") : t("settings.instance.access.saveCompanyAccess")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("settings.instance.access.currentMemberships")}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("settings.access.unset")} • {t(`statuses.${membership.status}`, { defaultValue: membership.status })}
                        </div>
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {new Date(membership.updatedAt).toLocaleDateString(getCurrentLocale())}
                      </div>
                    </div>
                  ))}
                </div>
