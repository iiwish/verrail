...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("agents.new.createFailed"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError(t("agents.new.openCodeModelRequired"));
        return;
      }
    }
    createAgent.mutate(
      ...
      <div className="border border-border">
          ...
          <Popover open={roleOpen} onOpenChange={setRoleOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {t(`agents.new.roles.${effectiveRole}`, { defaultValue: roleLabels[effectiveRole] ?? effectiveRole })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-36 p-1" align="start">
              {AGENT_ROLES.map((r) => (
                ...
                >
                  {t(`agents.new.roles.${r}`, { defaultValue: roleLabels[r] ?? r })}
                </button>
              ))}
            </PopoverContent>
          </Popover>
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("agents.new.companySkills")}</h2>
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("agents.new.firstAgentCeo")}</p>
          )}
          {formError && (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending ? t("agents.new.testing") : t("agents.new.test")}
                </Button>
                <Button
                  size="sm"
