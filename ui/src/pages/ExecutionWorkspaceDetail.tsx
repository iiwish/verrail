    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("workspace.execution.routines.defaultAgentSet") : t("workspace.execution.routines.chooseAgent")}</span>
          <span>{t("workspace.execution.routines.lastRun", { date: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt) })}</span>
      ...
      >
        ...
        {isRunning ? t("statuses.running") : t("routines.row.runNow")}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("workspace.execution.routines.started"),
        body: t("workspace.execution.routines.startedBody"),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.detail.errors.run"),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("workspace.execution.routines.title")}</CardTitle>
          <CardDescription>
            {t("workspace.execution.routines.description")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("workspace.execution.routines.loading")}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("workspace.execution.routines.loadFailed")}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("workspace.execution.routines.empty")}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("workspace.execution.errors.save"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("workspace.runtime.jobCompleted")
          : request.action === "stop"
            ? t("workspace.execution.runtime.stopped")
            : request.action === "restart"
              ? t("workspace.execution.runtime.restarted")
              : t("workspace.runtime.started"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("workspace.errors.control"));
    },
  ...
  const openWorkspace = useMutation({
    ...
    onError: async (error) => {
      ...
      setHandoffErrorMessage(
        failure ? null : error instanceof Error ? error.message : t("workspace.execution.errors.open"),
      );
  ...
  const repairWorkspace = useMutation({
    ...
    onSuccess: (result) => {
      ...
      setHandoffErrorMessage(null);
      setRuntimeActionErrorMessage(null);
      setRuntimeActionMessage(t("workspace.execution.runtime.repaired"));
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("workspace.execution.errors.repair"));
    },
  });
  ...
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("workspace.errors.load")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("workspace.execution.errors.buildUpdate"));
      return;
    }
    ...
    <>
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        ...
        <Tabs value={activeTab ?? "issues"} onValueChange={(value) => handleTabChange(value as ExecutionWorkspaceTab)}>
          <PageTabBar
            items={workspaceTabItems.map((item) => ({ value: item.value, label: isExecutionWorkspacePluginTab(item.value) ? item.label : t(`workspace.execution.tabs.${item.value}`) }))}
            align="start"
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("workspace.execution.runtime.noStartedServices")
                : t("workspace.execution.runtime.noConfig")
            }
            jobEmptyMessage={t("workspace.execution.runtime.noJobs")}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("workspace.execution.runtime.disabledHint")
            }
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("workspace.execution.settings.title")}</CardTitle>
                <CardDescription>
                  {t("workspace.execution.settings.description")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("workspace.execution.settings.retryClose") : t("workspace.execution.settings.close")}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("workspace.execution.settings.general")}</div>
                  <Field label={t("workspace.fields.name")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("workspace.execution.settings.namePlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("workspace.execution.settings.sourceControl")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("workspace.execution.settings.branchName")} hint={t("workspace.execution.settings.branchHint")}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("workspace.execution.settings.baseRef")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("workspace.execution.settings.paths")}</div>
                  <Field label={t("workspace.execution.settings.workingDirectory")}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("workspace.execution.settings.providerRef")}>
                    <Input
                      className="font-mono"
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("workspace.execution.settings.lifecycle")}</div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("workspace.execution.settings.runtimeConfig")}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("workspace.execution.settings.runtimeSource")}
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("workspace.execution.settings.runtimeOverride")
                            : runtimeConfigSource === "project_workspace"
                              ? t("workspace.execution.settings.runtimeInherited")
                              : t("workspace.execution.settings.runtimeNone")}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                      ...
                      >
                        {t("workspace.execution.settings.resetInherit")}
                      </Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("workspace.advancedJson")}</summary>
                    ...
                    <div className="mt-3">
                      <Field label="Workspace commands JSON" hint="Legacy `services` arrays still work, but `commands` supports both services and jobs.">
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("workspace.execution.settings.inheritRuntime")}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("workspace.execution.settings.servicePorts")}</div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("workspace.noUnsaved")}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("workspace.execution.context.title")}</CardTitle>
                <CardDescription>{t("workspace.execution.context.description")}</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <DetailRow label={t("workspace.execution.context.projectWorkspace")}>
              ...
              <DetailRow label={t("workspace.execution.context.sourceTask")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("workspace.execution.context.derivedFrom")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("workspace.execution.context.runtimeProvisioning")}>
                <RuntimeProvisionStatusValue
                  status={runtimeProvisionStatus}
                  onViewLogs={() => handleTabChange("runtime_logs")}
                />
              </DetailRow>
              <DetailRow label={t("workspace.fields.id")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("workspace.execution.location.title")}</CardTitle>
                <CardDescription>{t("workspace.execution.location.description")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("workspace.execution.settings.workingDirectory")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("common.none")}
              </DetailRow>
              <DetailRow label={t("workspace.execution.settings.providerRef")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("common.none")}
              </DetailRow>
              <DetailRow label={t("workspace.fields.repoUrl")}>
              ...
              <DetailRow label={t("workspace.execution.settings.baseRef")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("common.none")}
              </DetailRow>
              <DetailRow label={t("workspace.execution.location.branch")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("common.none")}
              </DetailRow>
              <DetailRow label={t("workspace.execution.location.opened")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("workspace.execution.location.lastUsed")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("workspace.execution.location.cleanup")}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("workspace.execution.location.notScheduled")}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("workspace.execution.logs.title")}</CardTitle>
              <CardDescription>{t("workspace.execution.logs.description")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("workspace.execution.logs.loading")}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("workspace.execution.logs.loadFailed")}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("workspace.execution.logs.empty")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            <SummarySlotCard
              ...
              title={t("workspace.execution.summary.title")}
              description={t("workspace.execution.summary.description")}
            />
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("workspace.loadingPlugin")}</CardContent>
          </Card>
          ...
          <MissingPluginTabPlaceholder
            ...
            defaultTabLabel={t("workspace.execution.backTasks")}
          />
        ) : activeTab === "routines" ? (
