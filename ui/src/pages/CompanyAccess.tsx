...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.access.members") },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("settings.access.toasts.updated"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("settings.access.toasts.updateFailed"),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("settings.access.toasts.joinApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("settings.access.toasts.joinApproveFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("settings.access.toasts.joinRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("settings.access.toasts.joinRejectFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("settings.access.toasts.removed"),
        body:
          result.reassignedIssueCount > 0
            ? t("settings.access.toasts.tasksCleaned", { count: result.reassignedIssueCount })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("settings.access.toasts.removeFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("settings.access.selectCompany")}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.access.loading")}</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("settings.access.forbidden")
  ...
  return (
    <div className="max-w-6xl space-y-8">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold">{t("settings.access.title")}</h1>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl bg-amber-500/10 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">
          {t("settings.access.instanceAdminNotice")}
        </div>
      )}
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("settings.access.pendingJoins")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("settings.access.pendingJoinsDescription")}
                </p>
              </div>
              <Badge variant="outline">{t("settings.access.pendingCount", { count: pendingHumanJoinRequests.length })}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("settings.access.unknownRequester")
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("settings.access.noEmail")
                  }
                  context={
                    request.invite
                      ? t("settings.access.inviteContext", { type: request.invite.allowedJoinTypes, role: request.invite.humanRole ? t(`settings.access.roles.${request.invite.humanRole}`) : t("settings.access.noDefaultRole") })
                      : t("settings.access.inviteUnavailable")
                  }
                  detail={t("settings.access.submitted", { date: new Date(request.createdAt).toLocaleString(getCurrentLocale()) })}
                  approveLabel={t("settings.access.approveHuman")}
                  rejectLabel={t("settings.access.rejectHuman")}
          ...
          <table className="w-full min-w-(--sz-44rem) text-left text-sm">
            <thead>
              <tr className="border-b border-border text-muted-foreground">
                ...
                <th className="px-3 py-2 font-medium">{t("settings.access.email")}</th>
                <th className="px-3 py-2 font-medium">{t("settings.access.role")}</th>
                <th className="px-3 py-2 font-medium">{t("settings.access.status")}</th>
                <th className="px-3 py-2 text-right font-medium">{t("settings.access.action")}</th>
              </tr>
            </thead>
            <tbody>
              {members.length === 0 ? (
                  ...
                  <tr key={member.id} className="border-b border-border last:border-b-0">
                    ...
                    <td className="px-3 py-3">
                      {member.membershipRole
                        ? t(`settings.access.roles.${member.membershipRole}`, { defaultValue: HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[member.membershipRole] })
                        : t("settings.access.unset")}
                    </td>
                    <td className="px-3 py-3">
                      <Badge variant={member.status === "active" ? "secondary" : member.status === "suspended" ? "destructive" : "outline"}>
                        {t(`statuses.${member.status}`, { defaultValue: member.status.replace("_", " ") })}
                      </Badge>
                    </td>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("settings.access.editMember")}</DialogTitle>
            <DialogDescription>
              {t("settings.access.editMemberDescription", { name: editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId })}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("settings.access.companyRole")}</span>
                  <select
                  ...
                  >
                    <option value="">{t("settings.access.unset")}</option>
                    {Object.entries(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS).map(([value, label]) => (
                      <option key={value} value={value}>
                        {t(`settings.access.roles.${value}`, { defaultValue: label })}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("settings.access.membershipStatus")}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? t("common.saving") : t("settings.access.saveMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("settings.access.removeMember")}</DialogTitle>
            <DialogDescription>
              {t("settings.access.removeMemberDescription", { name: memberDisplayName(removingMember) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("settings.access.checkingTasks")
                    : t("settings.access.openAssignedTasks", { count: assignedIssues.length })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("settings.access.taskReassignment")}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("settings.access.leaveUnassigned")}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("settings.access.humans")}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("settings.access.agents")}>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {t("settings.access.moreTasks", { count: assignedIssues.length - 6 })}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? t("settings.access.removing") : t("settings.access.removeMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings.company.title"), href: "/company/settings" },
      { label: t("settings.access.access") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.access.checkingExtensions")}</div>;
  }
    ...
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.access.advancedPermissions")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.access.advancedDescription")}
        </p>
      </div>

      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          <h2 className="text-sm font-semibold">{t("settings.access.advancedUnavailable")}</h2>
          <p className="text-sm text-muted-foreground">
            {t("settings.access.advancedUnavailableDescription")}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("settings.access.extensionsUnavailable", { error: errorMessage })}</p>
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild>
            <Link to="/company/settings/members">{t("settings.access.openMembers")}</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/company/settings/invites">{t("settings.access.openInvites")}</Link>
          </Button>
        </div>
      </div>
    </div>
