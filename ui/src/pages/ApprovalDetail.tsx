...
export function ApprovalDetail() {
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.detail.errors.reject")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.detail.errors.revision")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.detail.errors.resubmit")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.detail.errors.comment")),
  });
  ...
  const resolvedCta =
    primaryLinkedIssue
      ...
      : linkedAgentId
        ? {
            label: t("approvals.detail.openAgent"),
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvals.detail.confirmedDescription")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          >
            ...
            {t("approvals.detail.fullRequest")}
          </button>
          {showRawPayload && (
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-(length:--text-micro) text-muted-foreground mt-2">
              {t("approvals.detail.linkedTasksNotice")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            ...
            >
              {t("approvals.detail.deleteAgent")}
            </Button>
          )}
        </div>
      </div>
