    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle={t("costs.creditsDescription")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.net")}
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("timeline.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs.inferenceTokens", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("projects.detail.tabs.budget")}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.pausedSummary", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            <MetricTile
              ...
              subtitle={t("costs.debitCreditSummary", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costs.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs.estimatedInRange", { value: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectRange")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.budgetAmount", { value: formatCents(spendData.summary.budgetCents) })
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                ...
                <div className="space-y-4">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsHelp")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgetControl")}</CardTitle>
                  <CardDescription>
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.scopeDescriptions.company")
                            : scopeType === "agent"
                              ? t("costs.scopeDescriptions.agent")
                              : t("costs.scopeDescriptions.project")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noEventsInPeriod")}</p>
                  ) : (
