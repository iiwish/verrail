 ...
 * buffer. The `preface_missing` reason names a missing or an invalid HTTP/2
 * client connection preface inside the bounded readiness buffer (accepted
 * security fix 6): the host found no valid preface after the accepted READY
 * line, aborted the HTTP/2 open, and moved the run to `queue_v1` one time.
 */
